...
for dev in devices:
    ...
    # Draw the colorbar
    cbar=fig.colorbar(heatmap,ax=ax,fraction=.05,pad=.025)
    ...
    if ax==axes[-1]:
        cbar.ax.set_xlabel(r"$\frac{Detections}{"+str(min)+"\ min}$",fontsize=14,labelpad=10,rotation=90)
    ...
    ax.set_xticks(pts_x[1:-1])

    if ax == axes[-1]:
        ax.set_xticklabels(pts_x[1:-1],rotation=90)
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%d.%m'))
        ax.set_xlabel('Date [dd.mm]', labelpad=10)
    else:
        ax.set_xticklabels(['']*len(pts_x[1:-1]))
...
dev='LT1_LT2'
fig.suptitle(r"$\bf{Black\ grouse\ }$"+r"$\it{Lyrurus\ tetrix}$"+'\n', fontsize=18)
fig.tight_layout(rect=[0, 0.03, 1, 0.95])
png_path=basedir_img+'png/'+dev+'_nofill.png'
svg_path=basedir_img+'svg/'+dev+'_nofill.svg'
...
fig.clf()
fig.legend(handles, labels,bbox_to_anchor=(0., 0., 1, 1),loc='upper left')

png_path=basedir_img+'png/legend_nofill.png'
svg_path=basedir_img+'svg/legend_nofill.svg'
...
'''
fig_legend, ax = plt.subplots(1, 1,figsize=figsize,dpi=dpi)
fig_legend.legend(h,l,'center right')

# save the two figures to files

png_path=basedir_img+'png/legend.png'
svg_path=basedir_img+'svg/legend.svg'
fig_legend.savefig(png_path)
print(png_path,'Saved')
fig_legend.savefig(svg_path)
print(svg_path,'Saved')
'''
