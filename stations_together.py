...
basedir_img='/home/plaf2000/Documenti/Lagmut_Lyrtet/Images/stations/female/'
...
for dev in devices:
    ...
    last_row_ax=bool(i_ax/2>=2)
    second_col_ax=bool(i_ax%2>0)

    ax=axes[int(i_ax/2),i_ax%2]
        ...
        # if start and end recording times are not defined, it means it didn't record, so there's no need to do anything, we can skip to next date
            try:
                ...
                if type(this_date['FRAC_MIN']) is not np.float64:
                    ...
                    count['INDIVIDUALS']-=females['FEMALE']
    ...
    # Draw the colorbar
    cbar=fig.colorbar(heatmap,fraction=.05,pad=.025,ax=ax)

    # Match the colorbar's ticks with the right value
    # bins=bins_colorbar
    bin_options=[1,2,5,10,20,50,100]
    i_bins=0
    while zmax/bin_options[i_bins]>10:
        i_bins+=1
    bins=bin_options[i_bins]
    val_sel=np.linspace(0,zmax,zmax+2)
    cbar_labels=[''] * (zmax+1)
    cbar_vislabels=np.arange(0,zmax+1,bins).astype(int)
    cbar_labels[::bins] = cbar_vislabels
    val_sel+=.5
    cbar.set_ticks(val_sel[:-1])
    cbar.set_ticklabels(cbar_labels)
    if last_row_ax:
        cbar.ax.set_xlabel(r"$\frac{Calls}{"+str(min)+"\ min}$",labelpad=5,fontsize=14,rotation=90)
    ...
    x_labels=['']*len(xticks)
    if last_row_ax:
        x_labels=pd.to_datetime(xticks).strftime('%d.%m').to_numpy()

        x_labels[1::bins_date]=''
    ...
    label_minstart=minstart-(minstart%step_min)+step_min
    label_minend=minend-(minend%step_min)+step_min
    step_min/=2

    label_start=dt.datetime(2000,1,1,0,0,0)+dt.timedelta(minutes=label_minstart)
    label_end=dt.datetime(2000,1,1,0,0,0)+dt.timedelta(minutes=label_minend)

    step_tdelta=dt.timedelta(minutes=step_min)


    # Y ticks we want to show, referenced by minute
    y_hours_selected=np.arange(label_minstart,label_minend,step_min)
    ...
    hours_label=np.arange(label_start, label_end, step_tdelta).astype(dt.datetime)
    # Include last time value
    hours_label=hours_label[hours_label<=hend_dt]
    hours_label=pd.to_datetime(hours_label)
    hours_label=hours_label.strftime('%H:%M').to_numpy()
    hours_label[1::2]=''
    ax.set_yticks(y_hours_selected)
    if not second_col_ax:
        ax.set_yticklabels(hours_label)
        ax.set_ylabel('Time (UTC+2)', labelpad=10)
    else:
        ax.set_yticklabels(['']*len(y_hours_selected))
    ...
    ax.set_title(subtitle,fontsize=16,pad=10)
# plt.show()
fig.suptitle(r"$\bf{Rock\ ptarmigan\ }$"+r"$\it{Lagopus\ muta}$"+'\n',fontsize=18)

fig.tight_layout(h_pad=2,rect=[0, 0, 1, .95])
all_devs='_'.join([d for d in devices])
png_path=basedir_img+'png/'+all_devs+'_nofill.png'
svg_path=basedir_img+'svg/'+all_devs+'_nofill.svg'
