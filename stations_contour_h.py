...
for dev in devices:
    ...
    for d in dates:
        ...
        if not np.isnan(start_h) and not np.isnan(end_h):
            ...
            ax.vlines(xs[1],start_h,end_h,color=color['contour'],linestyle=':',linewidth=.5)
            if first:
                ax.vlines(xs[0],start_h,end_h,color=color['contour'],linestyle=':',linewidth=.5)
            ax.hlines([start_h,end_h],xs[0],xs[1],color=color['contour'],linestyle=':',linewidth=.5)
    ...
    heatmap=ax.pcolormesh(dates,y_hours,z,cmap=new_grays,vmin=0)
    ...
    mod=(hend-hstart)*60%step_min
    add_end=bool(mod/(hend-hstart)>=1.20)
    y_hours_selected=np.arange(hstart*60,hend*60+step_min,step_min)
    y_hours_selected=y_hours_selected[y_hours_selected<=hend*60]
    if add_end:
        y_hours_selected=np.append(y_hours_selected,hend*60)
    y_hours_selected=y_hours_selected/60
